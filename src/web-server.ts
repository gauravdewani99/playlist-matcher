...
export async function createWebServer(clientId: string, port: number = 3001) {
  ...
  // Debug endpoint to test matching for current user (temporary)
  app.get("/api/debug/test-match", requireAuth, async (req: AuthenticatedRequest, res) => {
    try {
      const spotifyClient = createSpotifyClient(req);
      const genreMatcher = new GenreMatcher(spotifyClient);
      const userId = req.userId || "unknown";

      console.log(`[Debug] Testing match for user ${userId}`);

      // Get current user
      const user = await spotifyClient.getCurrentUser();
      console.log(`[Debug] Spotify user: ${user.id}`);

      // Get liked songs
      const likedSongs = await spotifyClient.getLikedSongs(5);
      console.log(`[Debug] Liked songs fetched: ${likedSongs.length}`);

      // Get playlists
      const playlists = await spotifyClient.getUserPlaylists(10);
      console.log(`[Debug] Total playlists: ${playlists.length}`);
      const ownedPlaylists = playlists.filter(p => p.owner.id === user.id);
      console.log(`[Debug] Owned playlists: ${ownedPlaylists.length}`);

      // Get settings
      const settings = await settingsStore.getSettings(userId);

      // Try dry run matching
      const result = await genreMatcher.autoOrganize(5, 10, 0.15, true);

      res.json({
        userId,
        spotifyUserId: user.id,
        likedSongsCount: likedSongs.length,
        likedSongs: likedSongs.slice(0, 3).map(s => ({ name: s.name, artist: s.artists[0]?.name })),
        totalPlaylists: playlists.length,
        ownedPlaylistsCount: ownedPlaylists.length,
        ownedPlaylists: ownedPlaylists.slice(0, 5).map(p => ({ name: p.name, tracks: p.tracks.total })),
        settings,
        matchResult: {
          matches: result.matches.length,
          unmatched: result.unmatched.length,
          unmatchedReasons: result.unmatched.slice(0, 5).map(u => u.reason),
